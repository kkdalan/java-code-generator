...
public class BuildHelper {
	...
	return CommonValues.TAB + ScopeValues.PRIVATE.getValue() + cleanSpaces(attributeClass) + CommonValues.SPACE
		+ cleanSpaces(attributeName) + CommonValues.SEMICOLON;
    }
	...
	return (annotation.startsWith(ExpressionValues.AT.getValue()) ? cleanSpaces(annotation)
		: ExpressionValues.AT + cleanSpaces(annotation)) + CommonValues.NEWLINE;
    }
	...
	return CommonValues.TAB.getValue() + CommonValues.TAB.getValue() + bodyLine + CommonValues.NEWLINE.getValue();
    }

    public static String buildReturn(String returnString) {
	return CommonValues.TAB.getValue() + CommonValues.TAB.getValue() + ObjectValues.RETURN.getValue() + returnString
		+ CommonValues.SEMICOLON.getValue();
    }
	...
	constructor.append(CommonValues.TAB).append(scope)
		.append(returnStructure == null ? "" : returnStructure + CommonValues.SPACE).append(objectName)
		.append(CommonValues.PARENTHESIS);

	if (arguments != null && !arguments.isEmpty()) {
	    int position = 0;
	    for (Pair<Object, Object> obj : arguments) {
		constructor.append(obj.getKey()).append(CommonValues.SPACE).append(obj.getValue());
